		...
		<div class="w1000 cf">
			...
			<div class="wl">
				...
				<div class="analysis">
					...
					<p class="para">你于 <span class="cr">${stage_exam_time}</span>参加无涯测评的 <span class="cr">${stage_exam_ms.stage_exam_name}</span>，得分<span class="cr">${grade}</span>分/满分${stage_exam_ms.tatal_points }分。以下是对测评情况的详细分析。</p>
					<p class="para">你的<span class="cr">${stage_exam_ms.stage_exam_name}</span>测评能力位处<span class="cr">等级${rank}</span>，</p>
					<div class="cf">
						<div class="w380 l" style="margin:0 auto;">
							<table border="0" cellspacing="0" cellpadding="0" class="tbl">
								...
								<c:forEach items="${stageExamKnowlege}" var="s">
									<tr>
										<td title="Listening ">${s.knowlege_name} </td>
										<td>${s.rightCount}/${s.count}</td>
										<td>${s.rightGrade}/${s.grade}</td>
										<td>${s.rightGrade*100/s.grade}%</td>
									</tr>
								</c:forEach>
							</table>
						</div>
						...
						<a href="getUserQuestion.do?stage_exam_id=${stage_exam_id}&stage_grade_id=${stage_grade_id}" id="checkAnswer" class="btn btn-s">查看答案</a>
				</div>
				<div class="analysis">
					<div class="serial">
						<span></span>程度分析
