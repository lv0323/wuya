...
function findStage(){
	$.ajax({
		...
		success:function(data){
			$(".optiona").remove();
			for(var i = 0;i<data.length;i++){
				$("#stage").append("<option class='optiona'>"+data[i]+"</option>");
			} 
		}
	});
}
...
function submit_seek(){
	...
	var stage = $("#stage").val();
	$.ajax({
		url:"/WuYa/zbwstageexamlist.do",
		type:"post",
		data:"search="+search+"&subject="+subject+"&stage="+stage,
		dataType:"json",
		success:function(data){
			$(".selecttr").remove();
			for(var i = 0 ; i < data.length;i++){
				...
				$("#stage_exam_list").append(" <tr class='selecttr'><td><input type='checkbox' value='"+id+"'/><span>"+id+"</span></td><td class='center'>"+subject+"</td><td class='center'>"+week+"</td>"+
						...
						"<td class='center'><a href='${pageContext.request.contextPath}/zbwfindStageExamQuestion.do?id="+id+"' title='编辑'><img src='images/icon_edit.gif'/></a><a  title='删除' onclick='deleteExam("+id+")'><img src='images/icon_trash.gif'/></a>"+
						"</td></tr>");
				if(display==1){
					$("#33"+id).val(1);
				}else{
...
function hehe(id){
	...
	$.ajax({
		url:"/WuYa/zbwstageexamdisplay.do",
		type:"post",
 ...
 function deleteExam(id){
	 $.ajax({
		url:"/WuYa/zbwdeletestageexam.do",
		type:"post",
		data:"id="+id,
		dataType:"json",
		success:function(data){
...
<body>
 <div class="wrap">
  <div class="page-title">
    <span class="modular fl"><i></i><em>阶段测试试卷列表</em></span>
  </div>
   <div class="operate">
   <form>
    <select class="inline-select" id="subject" onchange="findStage()">
     <option>学科</option>
    </select>
    <select class="inline-select" id="stage">
     <option>阶段</option>
    </select>
    <input type="text" id="search" class="textBox length-long" placeholder="输入试卷名字..."/>
    <input type="button" value="清空" class="tdBtn" onclick="empty_seek()"/>
    <input type="button" value="查询" class="tdBtn" onclick="submit_seek()"/>
   </form>
  </div>
  <table class="list-style" id="stage_exam_list">
   <tr>
    <th>试卷编号</th>
    <th>所属学科</th>
    <th>对应阶段</th>
